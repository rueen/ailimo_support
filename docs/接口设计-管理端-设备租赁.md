...
### 1.5 审核通过设备租赁订单
**接口地址**: `PUT /api/support/equipment-reservations/:id/approve`
...
**权限要求**: `equipment_reservation:approve`

---

### 1.6 审核拒绝设备租赁订单
**接口地址**: `PUT /api/support/equipment-reservations/:id/reject`

**请求参数**:
```json
{
  "rejectReason": "该时间段设备需要维护"  // 必填，拒绝原因
}
```
...
**业务逻辑**:
1. 检查订单状态是否为"待审核"
2. **释放设备时间段**：由于订单是"待审核"状态，尚未占用设备，所以无需释放（订单状态为"进行中"或"已完成"时才占用设备）
3. 更新订单状态为"已拒绝"（2）
4. 记录拒绝原因、审核时间和审核人

**权限要求**: `equipment_reservation:reject`
...
### 3.7 获取提前预约天数配置
**接口地址**: `GET /api/support/equipment-advance-days`
...
**权限要求**: `equipment_advance_days:view`

---

### 3.8 更新提前预约天数配置
**接口地址**: `PUT /api/support/equipment-advance-days`
...
2. 更新system_configs表中的equipment_advance_days配置

**权限要求**: `equipment_advance_days:update`
