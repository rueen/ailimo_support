...
### 1.6 审核用户（通过）
**接口地址**: `PUT /api/support/users/:id/approve`
...
### 1.7 审核用户（拒绝）
**接口地址**: `PUT /api/support/users/:id/reject`

**请求参数**:
```json
{
  "rejectReason": "提交的资料不完整"  // 必填，拒绝原因
}
```
...
1. 检查用户当前审核状态是否为待审核
2. 更新审核状态为2（审核拒绝）
3. 记录拒绝原因（reject_reason）、审核时间（audit_time）和审核人（audit_by，当前登录的管理员）
