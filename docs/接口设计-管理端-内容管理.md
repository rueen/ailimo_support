...
```json
{
  ...
  "start_date": "2026-01-01", // 可选，开始日期（统计该日期之后创建的订单）
  "end_date": "2026-01-31"    // 可选，结束日期（统计该日期之前创建的订单）
}
```
...
3. 如果指定时间范围（start_date、end_date），只统计该时间范围内创建的订单
4. 统计各业务模块中 handler_id 等于该负责人的订单：
   - equipment（设备预约）
   - cage（笼位预约）
   - experiment（实验代操作）
   - animal（动物订购）
   - reagent（试剂耗材订购）
5. 统计状态：已完成（status=3）、进行中（status=1）
6. 按负责人创建时间倒序排列

**权限要求**: `handler:statistics`

**说明**: 
- 时间筛选基于订单的 `created_at` 字段
- 可单独使用 start_date 或 end_date，也可同时使用
- 用于统计每个负责人在各个业务模块完成的订单数据，可用于绩效考核、工作量统计等
