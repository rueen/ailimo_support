...
### 1.5 审核通过笼位租赁订单
**接口地址**: `PUT /api/support/cage-reservations/:id/approve`

**请求参数**:
```json
{
  "handlerId": 2  // 必填，负责人ID
}
```
...
**权限要求**: `cage_reservation:approve`

---

### 1.6 审核拒绝笼位租赁订单
**接口地址**: `PUT /api/support/cage-reservations/:id/reject`

**请求参数**:
```json
{
  "rejectReason": "该时间段笼位数量不足"  // 必填，拒绝原因
}
```
...
**业务逻辑**:
1. 检查订单状态是否为"待审核"
2. **释放笼位数量**：由于订单是"待审核"状态，尚未占用笼位数量，所以无需释放（订单状态为"进行中"或"已完成"时才占用笼位数量）
3. 更新订单状态为"已拒绝"（2）
4. 记录拒绝原因、审核时间和审核人

**权限要求**: `cage_reservation:reject`
...
### 4.7 获取提前预约天数配置
**接口地址**: `GET /api/support/cage-advance-days`
...
**权限要求**: `cage_advance_days:view`

---

### 4.8 更新提前预约天数配置
**接口地址**: `PUT /api/support/cage-advance-days`

**请求参数**:
```json
{
  "advanceDays": 7  // 必填，提前预约天数（单位：天）
}
```
...
**业务逻辑**:
1. 验证advanceDays必须大于0
2. 更新system_configs表中的cage_advance_days配置

**权限要求**: `cage_advance_days:update`
