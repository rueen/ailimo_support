...
### 2.7 查询笼位可用时间段
**接口地址**: `GET /api/support/cages/available-time-slots`
...
**说明**: 
- 该接口在 support 端和 h5 端都可使用
- H5端接口地址为 `GET /api/h5/cages/available-time-slots`
- 前端在创建订单时，应根据此接口返回的可用数量判断是否可以提交预约
- 如果用户选择的数量超过任一时间段的可用数量，应提示用户
