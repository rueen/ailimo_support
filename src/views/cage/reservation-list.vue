  ...
  <div class="cage-reservation-list-container">
    ...
    <div class="action-bar">
      <a-space>
        ...
        <template v-if="userStore.hasPermission('cage_reservation:audit') && selectedRowKeys.length > 0">
    ...
    <a-table
      ...
      :scroll="{ x: 1500 }"
      :row-selection="userStore.hasPermission('cage_reservation:audit') ? rowSelection : null"
      row-key="id"
...
const handleApproveSubmit = async () => {
  ...
  try {
    if (isBatchAudit.value) {
      const res = await batchAuditCageReservation({
